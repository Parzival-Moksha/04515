...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const success = await restoreSnapshot(id, session.user.id, snapshotId)
    if (!success) {
...
export async function PUT(_request: Request, context: RouteContext) {
  try {
    ...
    const worldState = await loadWorld(id, session.user.id)
    if (!worldState) {
    ...
    const { error } = await sb.from('world_snapshots').insert({
      world_id: id,
      data: worldState,
