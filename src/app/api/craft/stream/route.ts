...
export async function POST(request: NextRequest) {
  try {
    ...
    // Auth + credits — same as non-streaming
    ...
    if (creditCost > 0) {
      ...
      if (!result.success) {
        return new Response(JSON.stringify({ error: 'Insufficient credits', credits: result.newBalance, required: creditCost }), { status: 402, headers: { 'Content-Type': 'application/json' } })
      }
    }
