// ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
// ARIEL — The Spirit Who Executes The Wizard's Will
...
// ═══════════════════════════════════════════════════════════════════════════
// ARIEL'S SYSTEM PROMPT
// ═══════════════════════════════════════════════════════════════════════════

function buildSystemPreamble(): string {
  return `You are Ariel, the coding spirit of the Oasis (04515.xyz).
You are a full-power Claude Code agent with unrestricted read/write access to the repository.
The admin (Prospero) commands you through the Oasis UI.
...
- When done, tell the admin to hit F5 if the changes affect the running app

## CONTEXT
- Working directory: ${OASIS_ROOT}
- Stack: Next.js 14 + React Three Fiber + Three.js + Zustand
- Port: 4515
- You ARE the Oasis. You're editing yourself. The wizard circle is closed.`
}
...
export async function POST(request: NextRequest) {
  ...
  if (session.user.id !== ADMIN_USER_ID || !ADMIN_USER_ID) {
  ...
  console.log(`[Ariel] ${isResume ? 'Resuming' : 'New'} session (${agentModel}): "${prompt.substring(0, 80)}"`)
  ...
  const readable = new ReadableStream({
    start(controller) {
      ...
      sendEvent('status', { content: `Summoning Ariel (${agentModel})${isResume ? ' — resuming session' : ''}...` })
          ...
          try {
            ...
            if (eventType === 'system') {
              ...
              if (raw.subtype === 'init' && raw.session_id) {
                ...
                console.log(`[Ariel] Session: ${capturedSessionId}`)
      ...
      child.stderr.on('data', (chunk: Buffer) => {
        ...
        if (text.length > 0 && text.length < 2000) {
          console.log(`[Ariel:stderr] ${text.substring(0, 200)}`)
        }
      })
      ...
      child.on('error', (err) => {
        ...
        console.error(`[Ariel] Spawn error: ${err.message}`)
      ...
      child.on('close', (code) => {
        clearInterval(keepAliveInterval)
        console.log(`[Ariel] Process exited with code ${code}, cost=$${costUsd.toFixed(4)}`)
      ...
      request.signal.addEventListener('abort', () => {
        clearInterval(keepAliveInterval)
        console.log('[Ariel] Client disconnected, killing process')
        child.kill('SIGTERM')
      })
    },
  })
