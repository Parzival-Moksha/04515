...
export async function POST(request: NextRequest) {
  // Admin-only for v0.1
  if (!(await isAdmin())) {
    return NextResponse.json({ error: 'Forbidden — Merlin is admin-only in v0.1' }, { status: 403 })
  }
  ...
  // Load world — need owner user_id. For admin we use service role to load any world.
  ...
  const userId = worldRow.user_id as string
  const existingData = (worldRow.data || {}) as Partial<WorldState>
